...
const moduleId$8 = "crowdgoeswild";
const _VibeCheckPopup = class extends Application {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      popOut: true,
      template: `modules/${moduleId$8}/templates/VibeCheckPopup.hbs`,
      id: `${moduleId$8}-vibe-check`,
  ...
  async getData() {
    ...
    let data = {
      isGM: game.user.isGM,
      reactions: await game.settings.get(moduleId$8, "reactions"),
...
function registerSocketEvents() {
  game.socket.on(`module.${moduleId$7}`, handleSocketEvent);
}
async function emitSocketEvent({ type, payload }) {
  ...
  await game.socket.emit(`module.${moduleId$7}`, event);
...
async function initiateVibeCheck() {
  emitSocketEvent({
    type: "vibecheck",
    payload: { duration: game.settings.get(moduleId$7, "vibecheckduration") }
  });
}
...
async function loadReactionsPreset(presetName) {
  ...
  await game.settings.set(moduleId$6, "reactions", reactions);
  return;
}
function registerSettings() {
  console.log("Registering CGW Settings");
  game.settings.register(moduleId$6, "reactions", {
    name: "Reaction",
  ...
  game.settings.register(moduleId$6, "vibecheckautoclose", {
  ...
  game.settings.register(moduleId$6, "vibecheckduration", {
  ...
  game.settings.register(moduleId$6, "moduleVersion", {
    scope: "world",
    // "world" = sync to db, "client" = local storage
    config: false,
    // false if you dont want it to show in module config
  ...
  game.settings.register(moduleId$6, "maxdisplayed", {
...
class ReactionEditor extends FormApplication {
  ...
  async getThisReaction() {
    let reactions = await game.settings.get(moduleId$5, "reactions");
  ...
  activateListeners(html) {
    ...
    html.find(".colorSwitch").on("click", (ev) => {
      ev.stopPropagation();
      let i1 = $(ev.currentTarget).parents(".colors").find(".primaryColor color-picker").first().get(0);
      let i2 = $(ev.currentTarget).parents(".colors").find(".secondaryColor color-picker").first().get(0);
...
class ReactionSetupMenu extends Application {
  ...
  async getData() {
    ...
    data.currentReactions = game.settings.get(moduleId$4, "reactions");
    return data;
  }
  // switchColors(inputEl1, inputEl2) {
  //   let v1 = inputEl1.value;
  ...
  async exportReactions() {
    let data = await game.settings.get(moduleId$4, "reactions");
    let dataJSON = JSON.stringify(data);
    saveDataToFile(dataJSON, "text/json", "reactions.json");
  }
  async saveReactionSetData(data) {
...
async function getReactionObject(reactionId) {
  let reactions = await game.settings.get(moduleId$3, "reactions");
...
async function renderChatButtonBar() {
  ...
  let templateData = {
    reactions: await game.settings.get(moduleId$3, "reactions"),
    isGM: game.user.isGM
  };
